...
import {
  Shield, LayoutDashboard, Users, Settings, BarChart3,
  LogOut, Menu, ChevronRight, UserCircle2, Moon, Sun,
} from "lucide-react";
  ...
  const Sidebar = () => (
    <aside className="flex flex-col h-full bg-slate-950 text-white border-r border-white/10">
      <div className="flex items-center gap-3 px-5 py-5 border-b border-white/10 shrink-0">
        ...
        <div>
          <div className="font-bold text-sm tracking-tight">CyberShield <span className="text-cyan-400">Admin</span></div>
          <div className="text-[9px] uppercase tracking-[0.2em] text-slate-500">Command Center</div>
        </div>
      </div>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={cn(
                ...
                active ? "bg-cyan-500/15 text-white border border-cyan-500/20" : "text-slate-400 hover:bg-white/5 hover:text-white"
              )}
            >
              <item.icon className={cn("h-4 w-4 shrink-0", active ? "text-cyan-400" : "text-slate-600 group-hover:text-slate-300")} />
      ...
      <div className="shrink-0 border-t border-white/10 p-3 space-y-1">
        <div ref={profileRef} className="relative">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-cyan-500/20 border border-cyan-400/30 flex items-center justify-center text-cyan-300 text-xs font-bold shrink-0">
              {initials}
            </div>
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen">
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-slate-950 border-b border-white/10 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <div className="w-8 h-8 rounded-full bg-cyan-500/20 border border-cyan-400/30 flex items-center justify-center text-cyan-300 text-xs font-bold">
              {initials}
            </div>
          </div>
        </header>
