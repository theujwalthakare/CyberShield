...
import { Shield, Mail, KeyRound, Loader2, Save, Pencil, X, UserCircle2, CheckCircle2 } from "lucide-react";
...
export default function AdminProfilePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const p = { full_name: data?.full_name ?? "", email: data?.email ?? user.email ?? "", role: data?.role ?? "admin" };
      setProfile(p);
  ...
  if (loading) return <div className="flex h-[60vh] items-center justify-center"><Loader2 className="w-7 h-7 animate-spin text-cyan-400" /></div>;
    ...
    <div className="max-w-2xl mx-auto space-y-6 py-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{editing ? "Edit your admin account details." : "View your admin account details."}</p>
        </div>
        {!editing
      ...
      <div className="bg-slate-950 rounded-2xl p-6 flex items-center gap-5">
        <div className="w-16 h-16 rounded-full bg-cyan-500/20 border-2 border-cyan-400/40 flex items-center justify-center text-cyan-300 text-xl font-bold shrink-0">{initials}</div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            <Badge className="bg-cyan-500/20 text-cyan-300 border-cyan-500/30 text-[10px] uppercase font-bold hover:bg-cyan-500/20">Administrator</Badge>
            <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-300 bg-emerald-500/20 px-2 py-0.5 rounded-full"><CheckCircle2 className="w-3 h-3" /> Active</span>
          </div>
        </div>
        <Shield className="w-8 h-8 text-cyan-400/20 shrink-0" />
      </div>
      ...
      {editing && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-cyan-200 dark:border-cyan-900 shadow-sm p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="full_name" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Administrator name" />
          </div>
      ...
      {!editing && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm divide-y divide-slate-100 dark:divide-slate-800">
          ...
          {[
            ...
            { icon: KeyRound, label: "Role", value: "ADMINISTRATOR" },
