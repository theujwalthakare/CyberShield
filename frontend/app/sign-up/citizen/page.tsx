...
export default function CitizenSignUpPage() {
  const [form, setForm] = useState({
    fullName: "", email: "", phone: "", stateCode: "",
      ...
      <main className="relative mx-auto w-full max-w-lg px-4 py-10 sm:px-6">
        <section className="mb-6 rounded-2xl border border-cyan-500/20 bg-slate-900/75 p-6 backdrop-blur">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Submit complaints, track case status, and access safety resources.
          </p>
        </section>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2 space-y-1.5">
                      ...
                      <Input id="c-name" value={form.fullName} onChange={(e) => set("fullName", e.target.value)} placeholder="As per Aadhaar / ID" required />
                    </div>
                    <div className="space-y-1.5">
                      <Label htmlFor="c-email">Email *</Label>
                      <Input id="c-email" type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="you@example.com" required />
                    </div>
                    <div className="space-y-1.5">
                      <Label htmlFor="c-phone">Mobile *</Label>
                      <Input id="c-phone" type="tel" value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="+91 XXXXX XXXXX" required />
                    </div>
                    <div className="space-y-1.5">
                      <Label htmlFor="c-state">State Code</Label>
                      <Input id="c-state" value={form.stateCode} onChange={(e) => set("stateCode", e.target.value)} placeholder="e.g. MH, DL" />
                    </div>
                    <div className="space-y-1.5">
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="c-pwd">Password *</Label>
                      <div className="relative">
                        <Input id="c-pwd" type={showPwd ? "text" : "password"} value={form.password} onChange={(e) => set("password", e.target.value)} minLength={8} required />
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="c-cpwd">Confirm Password *</Label>
                      <Input id="c-cpwd" type="password" value={form.confirmPassword} onChange={(e) => set("confirmPassword", e.target.value)} minLength={8} required />
                    </div>
                  </div>
                  {error && <p className="rounded-md border border-rose-500/40 bg-rose-500/10 px-3 py-2 text-sm text-rose-200">{error}</p>}
