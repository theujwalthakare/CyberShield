...
import {
  Shield, LayoutDashboard, Activity, Search,
...
const navItems = [
  ...
  { href: "/analyst/intelligence", label: "Intelligence", icon: Search },
  { href: "/analyst/analytics", label: "Analytics", icon: Activity },
];

export default function AnalystLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = () => (
    <aside className="flex flex-col h-full bg-slate-950 text-white border-r border-white/10">
      <div className="flex items-center gap-3 px-5 py-5 border-b border-white/10 shrink-0">
        <div className="w-8 h-8 rounded-lg bg-violet-500/20 flex items-center justify-center">
          <Shield className="h-5 w-5 text-violet-400" />
        </div>
        <div>
          <div className="font-bold text-sm tracking-tight">CyberShield <span className="text-violet-400">Analyst</span></div>
          <div className="text-[9px] uppercase tracking-[0.2em] text-slate-500">Intelligence Lab</div>
        </div>
      </div>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={cn(
                ...
                active ? "bg-violet-500/15 text-white border border-violet-500/20" : "text-slate-400 hover:bg-white/5 hover:text-white"
              )}
            >
              <item.icon className={cn("h-4 w-4 shrink-0", active ? "text-violet-400" : "text-slate-600 group-hover:text-slate-300")} />
      ...
      <div className="shrink-0 border-t border-white/10 p-3 space-y-1">
        <div ref={profileRef} className="relative">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-violet-500/20 border border-violet-400/30 flex items-center justify-center text-violet-300 text-xs font-bold shrink-0">
              {initials}
            </div>
            <div className="flex-1 text-left min-w-0">
              <p className="text-sm font-medium text-slate-200 truncate">{name}</p>
              <p className="text-[10px] text-slate-500">Analyst Account</p>
            </div>
          ...
          {profileOpen && (
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-slate-900 border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
              ...
              <div className="p-1.5 space-y-0.5">
                <Link href="/analyst/profile" onClick={() => setProfileOpen(false)}
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen">
        <header className="lg:hidden flex items-center justify-between px-4 py-3 bg-slate-950 border-b border-white/10 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <div className="w-8 h-8 rounded-full bg-violet-500/20 border border-violet-400/30 flex items-center justify-center text-violet-300 text-xs font-bold">
              {initials}
            </div>
          </div>
        </header>
